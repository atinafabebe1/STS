...
const SidebarItemCollapse = ({ item }: Props) => {
  ...
  const childRoutes = item.child?.filter(
    (childRoute) => !childRoute.visibleOn || childRoute.visibleOn.includes(userRole)
  );
  ...
  return (
    <>
      ...
      >
        ...
        <ListItemText
          disableTypography
          primary={<Typography sx={{ paddingY: 0 }}>{item.sidebarProps?.displayText}</Typography>}
        />
